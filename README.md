...
- **Live on Swarm (M4):** every movement in a goal's history can carry a private context (shop, category, note), encrypted on a Swarm feed with a key derived from the user's Swarm ID identity. The feed manifest is published in the `formica.ledger` text record of `<user>.formica.eth` (Sepolia), so the ledger is discoverable from the name. Verified: a real note was written and read back, the on-chain record holds the manifest, and a public gateway resolves the feed returning opaque encrypted bytes — the note never appears in clear.
- **Web app:** M1 — dashboard, crea goal, checkout, incasso, withdraw su Fuji, tutti provati a mano ([`docs/e2e-m1.md`](docs/e2e-m1.md)). M2 — pagina Nomi (claim del namespace, nome per ogni goal) e incasso scrivendo un nome: i nomi on-chain sono stati creati e risolvono, la prova a mano della pagina è ancora aperta ([`docs/e2e-m2.md`](docs/e2e-m2.md)). M4 — libretto privato con contesto cifrato nello storico e pubblicazione del manifest nella pagina Nomi ([`docs/e2e-m4.md`](docs/e2e-m4.md)).
