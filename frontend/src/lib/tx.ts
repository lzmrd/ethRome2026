...
export type TxChainId = typeof SEPOLIA_CHAIN_ID | typeof FUJI_CHAIN_ID
...
export function useTx(chainId?: TxChainId) {
  const publicClient = usePublicClient(chainId === undefined ? undefined : { chainId })
  ...
  async function run(
    ...
    setPhase('simulating')
    try {
      // Le fee minime valgono solo su Fuji: su Sepolia la base fee e' nei gwei.
      const fujiFees = chainId === undefined || chainId === FUJI_CHAIN_ID
      const fees = fujiFees ? await currentFees(publicClient) : {}
