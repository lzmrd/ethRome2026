...
|---|---|---|---|---|
| 1 | Scrivo «bar testaccio · bar · ottimo caffè» su un movimento reale | l'app mostra il riferimento Swarm | ✅ la riga «Withdrawal 1.8 USDC» del vault `0x099c…42f5` mostra il contesto e il riferimento `d4fa6296…cf6e37` | nessuna tx: il libretto è fuori catena |
...
| 3 | Apro un browser diverso, stessa identità Swarm, ricarico | la nota ricompare | ✅ osservato: secondo browser, stessa origine `http://localhost:5173`, accesso all'identità `0xfede` con frase di recupero e password; «bar testaccio · bar · ottimo caffè» ricompare sulla riga «Withdrawal 1.8 USDC». Nessun dato locale condiviso fra i due browser: la nota è tornata dal feed | — |
...
| 5 | Identità Swarm non connessa | la pagina è identica a prima, con la barra che invita | ✅ osservato a schermo: barra «Connect Swarm ID», nessun errore, numeri da Fuji intatti | screenshot del 2026-09-12 |
...
- **Il libretto è legato all'origine dell'app.** `deriveAppSecret` è deterministico per account **e origine**, e la porta fa parte dell'origine: la stessa identità su `http://localhost:5174` deriverebbe chiavi diverse e leggerebbe un feed diverso, quindi vuoto. È il motivo per cui il punto 3 si prova sulla stessa porta, ed è un limite reale da dichiarare: un'app servita su un dominio diverso non vede i libretti scritti in locale. Va risolto legando il feed all'identità e non all'origine, e sta fra le cose da fare dopo l'hackathon.
